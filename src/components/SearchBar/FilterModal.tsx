...
import { View, Text, StyleSheet, Pressable, TouchableOpacity, Modal, FlatList, Button } from 'react-native';
...
const FilterModal = forwardRef<FilterModalRef>((_, ref) => {
  ...
  const [selectedDistrict, setSelectedDistrict] = useState<string>("Quận 6, Hồ Chí Minh");
  ...
  useEffect(() => {
    const fetchDistricts = async () => {
      const result = await getDistrictsByCityName("Hồ Chí Minh");
      setDistricts(result);
    };
  ...
  const handleSelectDistrict = (district: string) => {
    const location = `${district}, Hồ Chí Minh`;
      ...
      >
        ...
        <View className="flex-row flex-wrap items-center gap-4 px-4 py-6">
          {timeOptions.map((option) => (
            <Pressable
              ...
              className={`px-6 py-3 rounded-xl border ${
                isSelected(option) ? 'bg-[#5669ff] border-transparent' : 'border-gray-300 bg-white'
              }`}
            >
              <Text
                className={`text-2xl ${
          ...
          <Pressable
            ...
            className={`flex-row items-center px-6 py-3 rounded-xl border ${
              isSelected('Chọn ngày') ? 'bg-[#5669ff] border-transparent' : 'border-gray-300 bg-white'
            }`}
          >
            <Icon name="calendar-alt" size={32} color={isSelected('Chọn ngày') ? "#fff" : "#5669ff"} className="mr-2" />
            <Text className={`text-2xl mr-3 ${
                isSelected('Chọn ngày') ? 'text-white' : 'text-gray-600'
            }`}>
              {isDateSelected ? formatDate(date).split(' ')[0] : 'Chọn ngày'}
            </Text>
            <Icon name="chevron-right" size={16} color={isSelected('Chọn ngày') ? "#fff" : "#5669ff"} />
          </Pressable>
        </View>
        <Text className="text-2xl font-bold pl-4 mt-5">
          Vị trí
        </Text>
        <Pressable
        ...
        >
          <View className="flex-row items-center">
            ...
            <Text className="text-2xl">{selectedDistrict}</Text>
          </View>
        ...
        <View className="flex-1 flex-row mx-4 justify-between gap-4 mt-4 mb-8">
          ...
          >
            <Text className='font-semibold text-xl text-white text-center'>ÁP DỤNG</Text>
          </TouchableOpacity>
          <Modal
